    ...
    if Mod.query.count() < 10:
        try:
            ...
            for item in geode_mods:
                ...
                if not mod_title and item.get('versions') and isinstance(item['versions'], list) and len(item['versions']) > 0:
                    mod_title = item['versions'][0].get('name') or item['versions'][0].get('title')
                ...
                if isinstance(dev, dict):
                    mod_creator = dev.get('username') or dev.get('display_name') or 'Unknown Creator'
                else:
                    mod_creator = str(dev) if dev else 'Unknown Creator'
                    
                mod_desc = item.get('description') or 'No description available.'
                ...
                if not Mod.query.filter_by(mod_id=mod_id).first():
                    new_mod = Mod(
                        ...
                        description=mod_desc, downloads=mod_downloads, logo_url=logo
                    )
...
def index():
    ...
    if sort_by == 'az':
        query = Mod.query.order_by(Mod.title.asc())
    else:
        query = Mod.query.order_by(Mod.downloads.desc())
        
    mods_page = query.paginate(page=page, per_page=10, error_out=False)
    
    return render_template('index.html', mods_page=mods_page, sort_by=sort_by)
